/***********************************************************************************
 ...
 All rights reserved
  ...
  Find a radial curve for an input open NURBS curve

	Make:
	make radialc

	Run:
    radialc -i input_curve_file -x x_coord_of_radial_point -y y_coord_of_radial_point -z z_coord_of_radial_point
            -n number_of_segments_per_knot_span -s radial_scale_factor -o output_file
    Output: 
    Resulting radial curve in .VECT format
 
    example:
           prompt> radialc -i c.CURV -x 0.0 -y 0.0 -z 0.0  -n 50  -s 1.0  -o radialc.VECT
  *********************************************************************************/
...
void print_usage()
{
	fprintf(stderr, "usage: radialc -i input_curve_file -x x_coord_of_radial_point -y y_coord_of_radial_point -z z_coord_of_radial_point -n number_of_segments_per_knot_span -s radial_scale_factor -o output_file\n");
   ...
   used to draw each internal knot span of the curve */
{
  ...
  nspans = egeom->ncontpts - egeom->order + 1; /* because our input is an open NURBS curve */
...
void OutputCurvRadial(short nsegs, float scal, float *pt, float **norms,
		   double *kcurv, FILE *fo)
{
  ...
  for (i=0; i<nsegs; i++) {
    if (kcurv[i] != 0.0)
      ...
      k = 1.0e30;
    v[0] = pt[0] + k*norms[i][0];
    v[1] = pt[1] + k*norms[i][1];
    v[2] = pt[2] + k*norms[i][2];
...
int main(int argc, char* argv[])
{
	...
	FILE *fi, *fo;  
	ParCurv* egeom;
	char c, *input, *px, *py, *pz, *num_seg_span, *radial_scale, *output;

	short errflg = 0;
	input = px = py = pz = num_seg_span = radial_scale = output = 0;
	...
	if(fi = fopen(input, "r")) { /* input is the file name storing the input curve */
		...
		if(px && py && pz) {
			pt[0] = atof(px);	pt[1] = atof(py);	pt[2] = atof(pz); /* input constant radial point */		
	     ...
	     if(num_seg_span) { /* read the # of segments per knot span */
			...
			if(radial_scale) {
				...
				if(output) {
					if(fo = fopen(output, "w")) {
						...
						OutputCurvRadial(nsegs, radialScale, pt, norms, kcurv, fo);
				...
				fprintf(stderr, "Can't read the radial scale\n");
				print_usage();
				exit(-1);
			}
			...
			fprintf(stderr, "Can't read x, y, z coordinates of the radial point\n");
			print_usage();
			exit(-1);
		}
