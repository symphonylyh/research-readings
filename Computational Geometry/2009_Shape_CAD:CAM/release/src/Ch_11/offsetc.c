/***********************************************************************************
 ...
 All rights reserved
  ...
  Find an offset curve for an input open NURBS curve

	Make:
	make offsetc

	Run:
	offsetc -i input_curve_file	-n number_of_segments_per_knot_span	-d offset_distance	-o output_file

    Output: 
    Resulting offset curve in .VECT format
 
    example:
           prompt> offsetc -i c.CURV  -n 50  -d 1.0  -o offsetc.VECT
  *********************************************************************************/
...
void print_usage()
{
	fprintf(stderr, "usage: offsetc -i input_curve_file	-n number_of_segments_per_knot_span	-d offset_distance	-o output_file\n");
...
void OutputCurvOffset(short nsegs, float dist, float **pts, float **norms, FILE *fo)
{
  float v[3];
  short i;
  ...
  fprintf(fo, "M ");
  for (i=0; i<nsegs; i++) {
    v[0] = pts[i][0] + dist*norms[i][0];
    v[1] = pts[i][1] + dist*norms[i][1];
    v[2] = pts[i][2] + dist*norms[i][2];
...
int main(int argc, char* argv[])
{
	...
	FILE *fi, *fo;  
	ParCurv* egeom;
	char c, *input, *num_seg_span, *offset_dist, *output;

	short errflg = 0;
	input = num_seg_span = offset_dist = output = 0;

	if (argc > 1) {
		while ((c = getopt(argc, argv, "i:n:d:o:")) != -1)
			switch (c) {
			case 'i':
				input = optarg;
				break;
			...
			if(offset_dist) {
				...
				if(output) {
					if(fo = fopen(output, "w")) {
						...
						OutputCurvOffset(nsegs, offsetDist, pts, norms, fo);
