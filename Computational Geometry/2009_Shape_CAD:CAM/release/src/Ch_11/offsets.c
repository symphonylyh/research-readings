/***********************************************************************************
 ...
 All rights reserved
  ...
  Find an offset surface for an input open NURBS surface

	Make:
	make offsets

	Run:
	offsets -i input_surface_file	-m number_of_segments_per_u-knot_span -n number_of_segments_per_v-knot_span
	        -d offset_distance	-o output_file 

    Output: 
	Resulting offset surface in .VECT format 
 
    Example:
           prompt> offsets -i s.SURF  -m 20 -n 20  -d 0.3   -o offsets.VECT
  *********************************************************************************/
...
void print_usage()
{
	fprintf(stderr, "usage: offsets -i input_surface_file   -m number_of_segments_per_u-knot_span -n number_of_segments_per_v-knot_span  -d offset_distance   -o output_file\n");
...
void OutputSurfOffset(ParSurf *fgeom, short nsegu, short nsegv, short nsubu,
		   short nsubv, float dist, float ***pts, float ***norms, FILE *fo)
{
  vector *r, *norm, *tangu, *tangv;
  double du, dv, u, v, u0, u1, v0, v1;
  double kn, kx, nu, nv;
  ...
  for (i=0; i<nsegu; i++) {
	...
	fprintf(fo, "M ");
    for (j=0; j<nsegv; j++) {
      p[0] = pts[i][j][0] + dist*norms[i][j][0];
      p[1] = pts[i][j][1] + dist*norms[i][j][1];
      p[2] = pts[i][j][2] + dist*norms[i][j][2];
	...
	for (k=1; k<nsubv; k++) {
	  ...
	  r = GetSurfValues(fgeom, u, v, &tangu, &tangv, &norm, &kx, &kn,
			    &nu, &nv);
	  p[0] = r->x/r->w + dist*norm->x/norm->w;
	  p[1] = r->y/r->w + dist*norm->y/norm->w;
	  p[2] = r->z/r->w + dist*norm->z/norm->w;
	  free(r);
	  free(tangu);
	  free(tangv);
	  free(norm);
...
int main(int argc, char* argv[])
{
	...
	FILE *fi, *fo;  
	ParSurf* egeom;
	char c, *input, *num_seg_u, *num_seg_v, *offset_dist, *output;

	short errflg = 0;
	input = num_seg_u = num_seg_v = offset_dist = output = 0;

	if (argc > 1) {
		while ((c = getopt(argc, argv, "i:m:n:d:o:")) != -1)
			...
			if(offset_dist) {
				...
				if(output) {
					if(fo = fopen(output, "w")) {
						nsubu = nsubv = 1;
						OutputSurfOffset(egeom, nsegu, nsegv, nsubu, nsubv, offsetDist, pts, norms, fo);
				...
				fprintf(stderr, "Can't read the offset distance\n");
				print_usage();
				exit(-1);
			}
