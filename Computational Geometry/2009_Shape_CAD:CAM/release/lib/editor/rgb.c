...
void rainbow(double h, double s, double v, float *r, float *g, float *b)
{
  double trash;
  int i;
...
void dhsv2rgb(double h, double s, double v, float *r, float *g, float *b)
    /*...hexcone model...*/
   /* all variables in range [0,1] */
  /* here, h=.667 gives blue, h=0 or 1 gives red. */
{  /* see Alvy Ray Smith, Color Gamut Transform Pairs, SIGGRAPH '78 */
  double f, k, m, n, trash;
  int i;
  ...
  default:
    *r=1; *g=0; *b=0;
/** fprintf(stderr,"dhsv2rgb: bad i: %f %d",h,i); **/
/** exit(1); **/
  }
  f = *r;
  if( f < *g ) f = *g;
