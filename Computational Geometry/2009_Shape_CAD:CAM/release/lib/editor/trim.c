...
TrimSurf *CopyTrimSurf(TrimSurf *from, TrimSurf *to)
{
  ...
  if(!from->c_loops) { /* No explicit C-curve info */
	...
	to->c_loops[i].egeoms[j] = copyegeom(from->c_loops[i].egeoms[j], NULL);
    }
  }
  /******************* 11.1 end *********************/
...
TrimSurf *InvertTrimSurf(TrimSurf *from, TrimSurf *to)
{
  ...
  /******************* 11.1 begin **************************/
  if(from->c_loops) { /* C-curves available explicitly */ 
    to->c_loops = (TrimLoop *)gen_array1(to->nLoops, sizeof(TrimLoop));
  }
  else to->c_loops = NULL;
  /******************* 11.1 end ****************************/
  ...
  for (i=0,j=to->nLoops-1; j>=0; i++,j--) {
    ...
    /******************* 11.1 begin **************************/
    if(to->c_loops) { /* C-curves available explicitly */ 
      to->c_loops[i].nCurves = from->c_loops[j].nCurves;
      to->c_loops[i].egeoms = (ParCurv **)gen_array1(to->c_loops[i].nCurves,
						   sizeof(ParCurv *));
    }
    /******************* 11.1 end ****************************/
    ...
    /******************* 11.1 begin **************************/
    if(to->c_loops) { /* C-curves available explicitly */ 
      for (k=0,m=to->c_loops[i].nCurves-1; m>=0; k++,m--) {
	to->c_loops[i].egeoms[k] = copyegeom(from->c_loops[j].egeoms[m], NULL);
        RevCurvParam(to->c_loops[i].egeoms[k]);/* reverse the parametrization*/
      }
    }
    /******************* 11.1 end ****************************/
